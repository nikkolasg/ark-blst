...
impl ark_serialize::CanonicalDeserialize for Fp {
    fn deserialize_with_mode<R: ark_serialize::Read>(
        ...
        Option::from(blstrs::Fp::from_bytes_le(&buff))
            .map(|k| Fp(k))
...
impl ark_serialize::CanonicalDeserializeWithFlags for Fp {
    fn deserialize_with_flags<R: ark_serialize::Read, F: Flags>(
        ...
        Fr::from_u8(0).ok_or(SerializationError::InvalidData).map(|flag| Fp::deserialize_with_mode(reader, Compress::No, Validate::No).map(|elem| (elem,flag)))
    }
}
